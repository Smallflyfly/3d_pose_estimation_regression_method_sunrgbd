...
def im_detect(net, im):
  ...
  _, scores, bbox_pred, rois, _, grid_scores, rotation_scores = net.test_image(blobs['data'], blobs['im_info'])
  
  boxes = rois[:, 1:5] / im_scales[0]
  scores = np.reshape(scores, [scores.shape[0], -1])
  bbox_pred = np.reshape(bbox_pred, [bbox_pred.shape[0], -1])
  grid_scores = np.reshape(grid_scores, [grid_scores.shape[0], -1])
  rotation_scores = np.reshape(rotation_scores, [rotation_scores.shape[0], -1])
  ...
  return scores, pred_boxes, grid_scores, rotation_scores
