...
class Network(nn.Module):
  ...
  def _proposal_target_layer(self, rois, roi_scores):
    rois, roi_scores, labels, bbox_targets, bbox_inside_weights, bbox_outside_weights, depth_targets, rotation_targets, depth_inside_weights, depth_outside_weights = \
      proposal_target_layer(rois, roi_scores, self._gt_boxes, self._num_classes)    #fang fangpengfei add points_targets
    ...
    self._proposal_targets['depth_targets'] = depth_targets
    self._proposal_targets['rotation_targets'] = rotation_targets
    self._proposal_targets['depth_inside_weights'] = depth_inside_weights
    self._proposal_targets['depth_outside_weights'] = depth_outside_weights
  ...
  # def _huber_loss(self,points2d_pred, points2d_targets, delattr=1.0):
  #   y = points2d_pred.data.cpu().numpy()
  #   yt = points2d_targets.data.cpu().numpy()
  #   print(y)
  #   print(yt)
  #   # fang[-1]
  #   a = abs(y - yt)
  #   print(a)
    # return (a < delattr)*a**2/2.0 + (x > d) * d * (x-d/2.0)
  ...
  def _add_losses(self, sigma_rpn=3.0):
    ...
    depth_targets = self._proposal_targets['depth_targets']
    depth_inside_weights = self._proposal_targets['depth_inside_weights']
    depth_outside_weights = self._proposal_targets['depth_outside_weights']
    loss_depth = self._smooth_l1_loss(depth_pred, depth_targets, depth_inside_weights, depth_outside_weights)

    rotation_pred = self._predictions['rotation_pred']
    rotation_targets = self._proposal_targets['rotation_targets']
    rotation_inside_weights = depth_inside_weights
    rotation_outside_weights = depth_outside_weights
    loss_rotation = self._smooth_l1_loss(rotation_pred, rotation_targets, rotation_inside_weights, rotation_outside_weights)
    
    self._losses['cross_entropy'] = cross_entropy
    self._losses['loss_box'] = loss_box
    self._losses['rpn_cross_entropy'] = rpn_cross_entropy
    self._losses['rpn_loss_box'] = rpn_loss_box
    self._losses['loss_depth'] = loss_depth
    self._losses['loss_rotation'] = loss_rotation

    loss = cross_entropy + loss_box + rpn_cross_entropy + rpn_loss_box + loss_depth + loss_rotation  # add points2d loss add 0.1*loss_points2d
  ...
  def _region_classification(self, fc7):
    ...
    depth_pred = self.depth_pred_net(fc7)
    rotation_pred = self.rotation_pred_net(fc7)
    ...
    self._predictions["rotation_pred"] = rotation_pred

    return cls_prob, bbox_pred, depth_pred, rotation_pred
  ...
  def _init_modules(self):
    ...
    # net net net new net  i add net net fangpengfei  fangpengfei 
    self.depth_pred_net = nn.Linear(self._fc7_channels, self._num_classes * 1)  # 0- 99 + 100 + background 
    self.rotation_pred_net = nn.Linear(self._fc7_channels, self._num_classes * 1) #0 ~ 35 + 36 background
  ...
  def _predict(self):
    ...
    cls_prob, bbox_pred, depth_pred, rotation_pred = self._region_classification(fc7)  #fang fangpengfei add points2d
    
    for k in self._predictions.keys():
      self._score_summaries[k] = self._predictions[k]

    return rois, cls_prob, bbox_pred, depth_pred, rotation_pred  #fang fangpengfe add points2d
  ...
  def forward(self, image, im_info, gt_boxes=None, mode='TRAIN'):
    ...
    rois, cls_prob, bbox_pred, depth_pred, rotation_pred = self._predict()  #fang fang add points2d
    ...
    if mode == 'TEST':
      ...
      self._predictions["depth_pred"] = depth_pred
      self._predictions["rotation_pred"] = rotation_pred
      # self._predictions["points2d_pred"] = points2d_pred
      # print(self._predictions["bbox_pred"]) 300 * 28 7 * 4
      # print(self._predictions["points2d_pred"]) 300 * 133 7 * 19
    else:
  ...
  def init_weights(self):
    ...
    # fang fangpengfei add add add init
    normal_init(self.depth_pred_net, 0, 0.001, cfg.TRAIN.TRUNCATED)
    normal_init(self.rotation_pred_net, 0, 0.001, cfg.TRAIN.TRUNCATED)
  ...
  def test_image(self, image, im_info):
    ...
    cls_score, cls_prob, bbox_pred, rois, depth_pred, rotation_pred = self._predictions["cls_score"].data.cpu().numpy(), \
                                                     self._predictions['cls_prob'].data.cpu().numpy(), \
                                                     self._predictions['bbox_pred'].data.cpu().numpy(), \
                                                     self._predictions['rois'].data.cpu().numpy(),  \
                                                     self._predictions['depth_pred'].data.cpu().numpy(), \
                                                     self._predictions['rotation_pred'].data.cpu().numpy() #fang fangpengfei add points2d
    return cls_score, cls_prob, bbox_pred, rois, depth_pred, rotation_pred
  ...
  def train_step(self, blobs, train_op):
    self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'])
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss, loss_depth, loss_rotation = self._losses["rpn_cross_entropy"].data[0], \
                                                                        ...
                                                                        self._losses['loss_depth'].data[0], \
                                                                        self._losses['loss_rotation'].data[0] #add fang fangpengfei points2d_loss
    ...
    return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss, loss_depth, loss_rotation
  ...
  def train_step_with_summary(self, blobs, train_op):
    ...
    return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss, summary, loss_depth, loss_rotation
